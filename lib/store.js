...
export function createStore(dataDir) {
  ...
  return {
    ...
    distinctClusterIds({ geos = [], topics = [], hideTopics = [], outlets = [], hideOutlets = [], hours = null, q = null, notq = null, ids = null, limit = 50, offset = 0 } = {}) {
      ...
      const excludes = (notq || []).map((s) => s.toLowerCase());
      ...
      if (hideOutletsSet.size || hideTopicsSet.size || excludes.length) {
        entries = entries.filter(([cid]) => {
          ...
          if (excludes.length && all.some((x) => excludes.some((kw) => ((x.title || "") + " " + (x.summary || "")).toLowerCase().includes(kw)))) return false;
          return true;
        });
      }
      return entries
